...
{
  return std::abs(p1.x - p2.x) < 0.001 && std::abs(p1.y - p2.y) < 0.001 && std::abs(p1.z - p2.z) < 0.001;
}
} // namespace pcl
...
{
protected:
  typedef pcl::PointXYZ InputPointT;  // 点云点
  typedef pcl::PointXYZI VoxelPointT; // 珊格点
  typedef uint64_t IndexT;            // uin64，珊格点index
  ...
  DynamicVoxelGridTest()
      : origin_(1.0f, -2.0f, 0.0f), grid_(resolution_, 3), small_grid_(resolution_, 2), shifted_grid_(resolution_, 2, origin_)
  {
  }
...
TEST_F(DynamicVoxelGridTest, test_full)
{
  ...
  auto removed = grid_.removeIf([this](const VoxelPointT &p) {
    return grid_.getIndexOf(p) == grid_.getIndexOf(big_insert_1_[3]) || grid_.getIndexOf(p) == grid_.getIndexOf(big_insert_1_[5]);
  });
