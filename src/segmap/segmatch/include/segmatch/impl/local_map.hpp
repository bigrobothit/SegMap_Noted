...
LocalMap<InputPointT, ClusteredPointT>::LocalMap(
	...
	: voxel_grid_(params.voxel_size_m, params.min_points_per_voxel), radius_squared_m2_(pow(params.radius_m, 2.0)), min_vertical_distance_m_(params.min_vertical_distance_m), max_vertical_distance_m_(params.max_vertical_distance_m), normal_estimator_(std::move(normal_estimator))
{
...
std::vector<bool> LocalMap<InputPointT, ClusteredPointT>::updatePose(const laser_slam::Pose &pose)
{
	...
	std::vector<bool> is_point_removed = voxel_grid_.removeIf([&](const ClusteredPointT &p) {
		...
		bool remove = distance_xy_squared > radius_squared_m2_ || p.z - position.z < min_vertical_distance_m_ || p.z - position.z > max_vertical_distance_m_;
	...
	const std::vector<InputCloud> &new_clouds)
{
	...
	for (const auto &cloud : new_clouds)
		points_count += cloud.size();
