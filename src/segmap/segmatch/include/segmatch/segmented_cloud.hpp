...
{
  public:
    SegmentedCloud(const bool keep_only_last_view = true) : keep_only_last_view_(keep_only_last_view){};
                              ...
                              Id segment_id)
{
    ...
    {
        LOG_IF(ERROR, !g_too_many_segments_to_store_ids_in_intensity) << "Segment ID being passed to point intensity is larger than float "
    ...
    {
        CHECK_LT(index, reference_cloud.points.size()) << "Index is larger than the reference cloud size when adding segments. "
