...
SegMapper::SegMapper(ros::NodeHandle &n) : nh_(n)
{
	...
	for (unsigned int i = 0u; i < params_.number_of_robots; ++i)
	{
		...
		// 每一个robot都有一个local map，所有robot的local map使用vector存储
...
/// \brief 将所有机器人的local_map_取出构建出局部地图并发布
void SegMapper::publishMapThread()
{
	...
	while (ros::ok())
	{
		...
		for (size_t i = 0u; i < local_maps_.size(); ++i)
		{
			...
			local_maps += local_maps_[i].getFilteredPoints(); // 取出active的voxel grid中的cluster作为局部地图发布
			map_lock.unlock();
		}
...
/// \brief 对每一个robot发布world 到 odom的tf
...
void SegMapper::segMatchThread()
{
	...
	unsigned int track_id = laser_slam_workers_.size() - 1u; // robot id
	...
	while (ros::ok())
	{
		...
		// 根据每个robot的pose和获取的对应点云，叠加到每个机器人的local map中
		...
		else //回环模式
		{
												 ...
												 track_id, &loop_closure))
			{
				...
				LOG(INFO) << "Found loop closure! track_id_a: " << loop_closure.track_id_a << " time_a_ns: " << loop_closure.time_a_ns << " track_id_b: " << loop_closure.track_id_b << " time_b_ns: " << loop_closure.time_b_ns;
