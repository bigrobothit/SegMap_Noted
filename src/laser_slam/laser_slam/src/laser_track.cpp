...
LaserTrack::LaserTrack(const LaserTrackParams &parameters, unsigned int laser_track_id) : params_(parameters),
...
/// \brief: laser slam核心tracking节点，里面对时间同步进行了处理
/// \param pose: 帧间预测量
/// \param in_scan: 当前帧lidar点云
...
void LaserTrack::localScanToSubMap()
{
    ...
    for (size_t i = 0u; i < std::min(getNumScans() - 2u, size_t(params_.nscan_in_sub_map - 1u)); ++i)
    {
        ...
        transformation_matrix = (T_w_to_second_last_scan.inverse() *
                                 trajectory_.evaluate(previous_scan.time_ns))
    ...
    if (params_.save_icp_results)
    {
        ...
        correctTransformationMatrix(&transformation_matrix);
        rigid_transformation_->compute(last_scan.scan, transformation_matrix).save("/tmp/last_scan_alligned_by_initial_guess.vtk");
        correctTransformationMatrix(&icp_solution);
        rigid_transformation_->compute(last_scan.scan, icp_solution).save("/tmp/last_scan_alligned_by_solution.vtk");
    }
...
{
    std::lock_guard<std::recursive_mutex> lock(full_laser_track_mutex_);
    LOG(INFO) << "buildSubMapAroundTime " << time_ns << " for track " << laser_track_id_;
        ...
        for (unsigned int i = 0u; i < sub_maps_radius; ++i)
        {
            if (!reached_begin)
            {
                ...
                transformation_matrix = (T_w_a.inverse() *
                                         trajectory_.evaluate(it_before->time_ns))
                                            .getTransformationMatrix()
                                            .cast<float>();
                correctTransformationMatrix(&transformation_matrix);
    ...
    for (unsigned int i = 0u; i < sub_maps_radius; ++i)
    {
        ...
        {
            transformation_matrix = (T_w_a.inverse() *
                                     trajectory_.evaluate(it_after->time_ns))
                                        .getTransformationMatrix()
                                        .cast<float>();
            correctTransformationMatrix(&transformation_matrix);
