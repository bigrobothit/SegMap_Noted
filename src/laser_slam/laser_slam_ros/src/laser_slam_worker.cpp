	...
	unsigned int worker_id)
{
	...
	// 订阅ground true，用于实验对比
	gt_pose_sub_ = nh.subscribe("tf", 10, &LaserSlamWorker::tfCallback, this);
	...
	// 本质是设置velodyne到odom(数据集中是world)的trans
...
/// \brief 读取kitti bag中的ground true
void LaserSlamWorker::tfCallback(const tf::tfMessageConstPtr &msg)
{
	ROS_ERROR_STREAM("sync callback");

	tf::Transform trans1, trans2, trans3;
	int cnt = 0;
	for (const auto &it : msg->transforms)
	{
		if (it.header.frame_id == "world" && it.child_frame_id == "imu")
		{
			trans1.setOrigin(tf::Vector3(it.transform.translation.x, it.transform.translation.y, it.transform.translation.z));
			trans1.setRotation(tf::Quaternion(it.transform.rotation.x, it.transform.rotation.y, it.transform.rotation.z, it.transform.rotation.w));
			cnt++;
		}

		if (it.header.frame_id == "imu" && it.child_frame_id == "velodyne")
		{
			trans2.setOrigin(tf::Vector3(it.transform.translation.x, it.transform.translation.y, it.transform.translation.z));
			trans2.setRotation(tf::Quaternion(it.transform.rotation.x, it.transform.rotation.y, it.transform.rotation.z, it.transform.rotation.w));
			cnt++;
		}
	}
	if (cnt != 2)
	...
	pose_msg_.header.stamp = msg->transforms[0].header.stamp;
	pose_msg_.header.frame_id = "map";
	pose_msg_.pose.position.x = trans3.getOrigin().getX();
	pose_msg_.pose.position.y = trans3.getOrigin().getY();
	pose_msg_.pose.position.z = trans3.getOrigin().getZ();
	pose_msg_.pose.orientation.x = trans3.getRotation().getX();
	pose_msg_.pose.orientation.y = trans3.getRotation().getY();
	pose_msg_.pose.orientation.z = trans3.getRotation().getZ();
	pose_msg_.pose.orientation.w = trans3.getRotation().getW();

	// pose_pub.publish(pose_msg_);
	// f << std::setprecision(13)<< msg->transforms[0].header.stamp.toSec() << " "
...
void LaserSlamWorker::scanCallback(const sensor_msgs::PointCloud2 &cloud_msg_in)
{
	...
	if (!lock_scan_callback_)
	{
										  ...
										  cloud_msg_in.header.stamp, ros::Duration(kTimeout_s)))
		{
			...
			// 得到帧间odom的T，帧间运动超过一定阈值，才进行slam， 这个是为了防止添加过多的Graph nodes，控制计算量
			if (!last_pose_set_)
			{
				process_scan = true;
				last_pose_set_ = true;
			...
			// 准许进行slam，将当前点云数据和odom数据加入到laser tracking中
			if (process_scan)
			{
				// 点云数据格式转换
				...
				// 通过tracking得到map到sensor_frame，通过odom信息得到odom到sensor_frame
				// 计算map到odom的transform，存储在world_to_odom_
				Pose current_pose = laser_track_->getCurrentPose();
									 ...
									 const laser_slam::Time last_pose_before_update_timestamp_ns)
{
	...
	const Eigen::Matrix4f transform_matrix = (new_last_pose * last_pose_before_update.inverse()).getTransformationMatrix().cast<float>();
	{
		std::lock_guard<std::recursive_mutex> lock(local_map_mutex_);
